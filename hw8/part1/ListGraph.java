...
public class ListGraph extends AbstractGraph {
    ...
    public Edge getEdge(int source, int dest){
        Edge target = new Edge(source, dest, Double.POSITIVE_INFINITY,Double.POSITIVE_INFINITY,Double.POSITIVE_INFINITY,Double.POSITIVE_INFINITY);
    ...
    private Edge parseFileString(String input)
    {
        ...
        { //in case we fail to parse anything, such as format didn't work
            ...
            double distance = Edge.UNWEIGHTED_EDGE;
            double time = Edge.UNWEIGHTED_EDGE;
            double quality = Edge.UNWEIGHTED_EDGE;
            if(sc.hasNextDouble())
                weight = sc.nextDouble(); //parse in the weight if it's there
            toReturn = new Edge(source, dest, weight,distance, time,quality);
        }
        catch(Exception e)
        {
