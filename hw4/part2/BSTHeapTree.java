...
public class BSTHeapTree<E extends Comparable<E>>
    ...
    protected ArrayList<Integer> occount;
    ...
    {
        root=null;
        arr=new ArrayList<Integer>(3000);
        occount=new ArrayList<Integer>(3000);
    }
    ...
    private Node<E> add(Node<E> localRoot,  E item){
        ...
        if(localRoot == null){
            ...
            addReturn = true;
            return new Node<E>(new MaxHeap<E>());
        } else {
            if(localRoot.data.getTheData().size()<=7)
            {
                localRoot.data.add(item);
               // System.out.println("size :"+ localRoot.data.getTheData().size());
                return localRoot;
            }
            else
            {
                int compare = item.compareTo(localRoot.data.getTheData().get(0));
                if ( compare==0){
                    //item is equal to localRoot.dat
    ...
    public E find(E target) {
       ...
       return  (E) occount.get((Integer) target);
    }
    ...
    private Node<E> find(Node<E> localRoot, E target){
        if(localRoot == null)
            return null;
        for(int i =0;i<localRoot.data.getTheData().size();i++)
        {
            if( (target.compareTo(localRoot.data.getTheData().get(i))==0))
            {
        ...
        int compResult = target.compareTo(localRoot.data.getTheData().get(0));
        if(compResult == 0)
            return localRoot;
    ...
    private Node<E> delete(Node<E> localRoot, E item){
        ...
        for(int i=0; i<localRoot.data.getTheData().size();++i)
        {
            ...
            if((item.compareTo(localRoot.data.getTheData().get(i)))==0)
            {

                localRoot.data.getTheData().remove(i);
                return localRoot;
            }
        }

        int compResult = item.compareTo(localRoot.data.getTheData().get(0));
        if(compResult < 0){
          ...
          //  deleteReturn = localRoot.data;
            if(localRoot.left == null){
                ...
                if(localRoot.right.data.getTheData().size() !=1)
                {
                    localRoot.right.data.getTheData().remove(0);
                ...
                if(localRoot.left.data.getTheData().size() !=1)
                {
                    localRoot.left.data.getTheData().remove(0);
                ...
                if(localRoot.left.right == null){
                    ...
                    if(localRoot.data.getTheData().size() !=1)
                    {
                        localRoot.data.getTheData().remove(0);
                        return localRoot;
                    }
                    else
                    {
